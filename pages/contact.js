...
export default function Contact() {
  const validationSchema = Yup.object().shape({
    name: Yup.string()
      ...
      .max(
        maxInputLength,
        `* Name must be at most ${maxInputLength} characters.`
      ),
    email: Yup.string()
      .required("* Email is required")
      .max(
        maxInputLength,
        `* Email must be at most ${maxInputLength} characters.`
      )
        ...
        >
          ...
          <div>
            <div>
              ...
              />
              <div className={utilStyles.errorMessage}>
                {errors.name?.message}
              </div>
            </div>
            <div>
            ...
            <div
              style={{ float: "right", display: "flex", paddingBottom: "25px" }}
            >
              {emailSent && (
                <div style={{ paddingRight: "50px", marginTop: "10px" }}>
                  ✔️
                </div>
              )}
              {loading && !emailSent && (
                <ThreeDots className={utilStyles.formLoader} stroke="grey" />
              )}
              {!loading && !emailSent && (
              ...
              <button
                ...
                className={utilStyles.resetButton}
              >
                Reset
              </button>
            </div>
